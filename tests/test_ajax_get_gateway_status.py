...
def test_get_gateway_status(caplog, requests_mock, response, status_code, usage_point_id):
    ...
    if usage_point_id:
        assert f"INFO     root:dependencies.py:88 [{usage_point_id.upper()}] CHECK DE L'ÉTAT DE LA PASSERELLE.\n" in caplog.text
    else:
        assert "INFO     root:dependencies.py:88 CHECK DE L'ÉTAT DE LA PASSERELLE.\n" in caplog.text